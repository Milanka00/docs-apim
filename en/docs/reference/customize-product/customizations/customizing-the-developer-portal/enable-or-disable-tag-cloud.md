...
1. Open the `<API-M_HOME>/repository/deployment/server/webapps/devportal/site/public/theme/userTheme.json` file in a text editor and set the `tagCloud.active` attribute as `false`.
...
| leftMenu.color | string | | Set the color for the text in left side panel shown when tag cloud or tag wise grouping is visible | 
