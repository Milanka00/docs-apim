...
By using `defaultTheme.js` as a reference , you can customize other properties by modifying `userTheme.json` file.

1. Open the `<API-M_HOME>/repository/deployment/server/webapps/devportal/site/public/theme/userTheme.json` file in a text editor and set the `banner.active` attribute as `true` to show the banner.
